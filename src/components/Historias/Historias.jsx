...
import HISTORIAS from "./historiasData";
    ...
    <>
          ...
          <div className="story-rail mt-7 flex gap-6 overflow-x-auto pb-3 pt-1 sm:gap-8">
            {colecciones.map((item, index) => (
              <button
                ...
                type="button"
                onClick={() => abrir(item.id)}
                className="story-cover group flex w-[5.75rem] shrink-0 flex-col items-center gap-3 text-center"
                aria-label={`Abrir historias: ${item.title}`}
                data-reveal
                style={{ "--reveal-delay": `${index * 80}ms` }}
              >
                <span className="story-cover-ring relative grid size-[5.25rem] place-items-center rounded-full p-[3px]">
      ...
      {historia && slide &&
        ...
        >
          ...
          <article className="story-viewer relative h-[100svh] w-full max-w-[31rem] overflow-hidden bg-ag-card sm:h-[min(86svh,54rem)] sm:aspect-[9/16] sm:w-auto">
            <img
              key={slide.id}
              src={slide.src}
              alt={slide.alt}
            ...
            <div className="absolute inset-x-0 top-0 z-10 px-4 pt-4 sm:pt-5">
                      ...
                      style={
                        index === slideActivo
                          ? {
                              ...
                              animationPlayState: pausada ? "paused" : "running",
                            }
                          : undefined
                      }
                    />
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <button
                  ...
                  aria-label={pausada ? "Reanudar historia" : "Pausar historia"}
                >
