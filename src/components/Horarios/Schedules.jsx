...
const ESTILOS_CLASE = {
  maquina: "border-maquina-border bg-maquina-bg text-maquina-text",
  aereo: "border-aereo-border bg-aereo-bg text-aereo-text",
  funcional: "border-funcional-border bg-funcional-bg text-funcional-text",
  barre: "border-barre-border bg-barre-bg text-barre-text",
  recuperacion:
    "border-recuperacion-border bg-recuperacion-bg text-recuperacion-text",
  semiprivada:
    "border-semiprivada-border bg-semiprivada-bg text-semiprivada-text",
  default: "border-default-border bg-default-bg text-default-text",
  aereo: "border-aereo-border bg-aereo-bg text-aereo-text",
  funcional: "border-funcional-border bg-funcional-bg text-funcional-text",
  barre: "border-barre-border bg-barre-bg text-barre-text",
  recuperacion:
...
const diaInicial = () => {
  const indice = new Date().getDay() - 1; // Ajuste para que lunes sea 0, martes 1, ..., domingo 6
  return DIAS_SEMANA[indice]?.id ?? "lunes";
};

const coincideFranja = (hora, franja) => {
  if (franja === "todas") return true; // Si la franja es "todas", siempre coincide
  const inicio = Number.parseInt(hora.slice(0, 2), 10); // Extrae la hora como número entero
  return franja === "manana" ? inicio < 14 : inicio >= 17; // Compara según la franja
};
...
const Schedules = () => {
  ...
  const horariosAgrupados = useMemo(() => {
    ...
    return [...agrupados.values()].sort((a, b) => a.hora.localeCompare(b.hora));
  }, [horarios]);
    ...
    >
      ...
      <div className="liquid-glass-soft mt-7 grid gap-5 rounded-lg p-5 md:grid-cols-[minmax(180px,0.65fr)_minmax(320px,1fr)_auto] md:items-end">
        <label className="block">
          <span className="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-muted-text">
                ...
                <button
                  ...
                  className={`border-r border-ag-border px-2 text-xs transition-colors last:border-r-0 ${
                    activa
                      ...
                      : "bg-transparent text-muted-text-light hover:text-white"
                  }`}
                >
                  {franja.label}
                </button>
            ...
            return (
              <button
                ...
                className={`h-12 min-w-0 border-r border-ag-border text-xs uppercase transition-colors last:border-r-0 ${
                  activo
                    ? "bg-ag-gold font-semibold text-black"
                    : "bg-transparent text-muted-text-light"
                }`}
              >
                {dia.corto}
              </button>
        ...
        <div className="mt-3 divide-y divide-ag-border border-y border-ag-border">
          {clasesDiaSeleccionado.length > 0 ? (
            clasesDiaSeleccionado.map((clase, index) => (
              ...
              >
                <div className="flex items-start gap-2 text-sm text-muted-text-light">
                  <FontAwesomeIcon
                    icon={faClock}
          ...
          <thead>
            <tr className="bg-ag-card ">
              ...
              {DIAS_SEMANA.map((dia) => (
                <th
                  key={dia.id}
                  className="border-b border-r border-ag-border px-3 py-4 text-center text-xs font-medium uppercase tracking-[0.16em] text-muted-text-light last:border-r-0"
                >
                  {dia.nombre}
                </th>
              ))}
          ...
          <tbody>
            {filasFiltradas.length > 0 ? (
              filasFiltradas.map((fila) => (
                <tr key={fila.hora} className="even:bg-white/1.5">
                  <td className="border-b border-r border-ag-border px-4 py-4 align-top font-display text-lg text-muted-text-light">
                    {fila.hora}
                  </td>
                  {DIAS_SEMANA.map((dia) => (
                    <td
                      ...
                      {fila.dias[dia.id].length > 0 ? (
                        <div className="space-y-2">
                          {fila.dias[dia.id].map((clase, index) => (
                            ...
                            >
                              <strong className="block text-xs font-semibold uppercase tracking-widest">
                                {clase.nombre}
                              </strong>
                              <span className="mt-4 flex items-center justify-between text-[10px] uppercase tracking-widest opacity-80">
                                Consultar plaza
